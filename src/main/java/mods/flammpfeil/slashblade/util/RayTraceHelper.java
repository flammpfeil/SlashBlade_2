...
public class RayTraceHelper {
    ...
    public static Optional<RayTraceResult> rayTrace(World worldIn, Entity entityIn, Vec3d start, Vec3d dir, double blockReach, double entityReach, Predicate<Entity> selector){
        Vec3d end = start.add(dir.scale(blockReach));
    ...
    @Nullable
    public static EntityRayTraceResult rayTrace(World worldIn, Entity entityIn, Vec3d start, Vec3d end, AxisAlignedBB boundingBox, Predicate<Entity> selector){
        return rayTrace(worldIn, entityIn, start, end, boundingBox, selector, Double.MAX_VALUE);
    }

    @Nullable
    public static EntityRayTraceResult rayTrace(World worldIn, Entity entityIn, Vec3d start, Vec3d end, AxisAlignedBB boundingBox, Predicate<Entity> selector, double limitDist) {
        ...
        for(Entity foundEntity : worldIn.getEntitiesInAABBexcluding(entityIn, boundingBox, selector)) {
            ...
            Optional<Vec3d> optional = axisalignedbb.rayTrace(start, end);
